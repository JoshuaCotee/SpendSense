...
export const AddTransaction: React.FC<AddTransactionProps> = React.memo(({ onClose, navigation }) => {
  ...
  useEffect(() => {
    ...
    } else if (availableCategories.length > 0 && !availableCategories.includes(category)) {
      setCategory(availableCategories[0]);
    }
        ...
        <DatePicker
          ...
          onConfirm={(selected) => {
            ...
            if (selected > now) {
              showAlert({
                title: "Invalid Date",
                message: "You cannot add transactions in the future.",
                type: "error",
              });
              return;
            }
